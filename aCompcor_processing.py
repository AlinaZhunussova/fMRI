# After creating GM, WM, and CSF probabilistic maps using SPM pipeline,
# I use fmriprep function aCompCorMasks from https://github.com/nipreps/fmriprep/blob/9f426f7ad408c4f28691c3db2b9ca30dc42e8435/fmriprep/utils/confounds.py#L61
...
base_dir = Path("/my_path/skull_stripping_spm_thresh05") # directory with c1, c2, and c3 masks from SPM
...
for subj_id in subjects:
    ...
    anat_dir = base_dir / subj_id / "anat"
    in_files = [
        str(anat_dir / "c1T1w.nii"),
        str(anat_dir / "c2T1w.nii"),
        str(anat_dir / "c3T1w.nii")
    ]
    
    if not all(Path(f).exists() for f in in_files):
        print("Missing files")
        continue
    
    output_dir = anat_dir / "compcor_masks"
    output_dir.mkdir(exist_ok=True)
    os.chdir(str(output_dir))
    
    try:
        masks = acompcor_masks(in_files, is_aseg=False)
        print("✓")
